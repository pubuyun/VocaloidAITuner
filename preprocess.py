    ...
    try:
        for track in vtracks.vsTrack:
            for part in track.vsPart:
                ...
                ccs = part.VCC
                notes = part.VNote
                length = max(int(ccs[-1].t) if ccs!=[] else 0, (int(notes[-1].t) + int(notes[-1].dur)) if notes!=[] else 0)
                print(length)
                ...
                if random.random()<0.1 or m==1:
                    df.to_csv('testfiles/data'+str(m)+'.csv', index=False)
                    m+=1
                else:
                    df.to_csv('trainfiles/data'+str(n)+'.csv', index=False)
                    n+=1
    except Exception as e:
        print('Error in '+str(j), f'{e}')
        continue
